...
function get(row, ...names) {
  for (const n of names) {
    ...
    if (row && row[up] !== undefined) return row[up];
    if (row && row[low] !== undefined) return row[low];
  }
  return null;
}
...
// ================== REGRA NOVA: DATAS SEMPRE SYSDATE E BATENDO ==================
...
function getSysDates() {
  // pega o "agora" uma vez por execução (ou por grupo), para bater 100%
  const now = new Date();
...
// ✅ DATAS SEMPRE SYSDATE E IGUAIS: dataAssinaturaContrato, MMYYYY1Pagamento, dataCadastroLoteContrato

function buildDadosFromGrupo(grupoRows, cepInfoTitular, titularRow, sysDates) {
  if (!titularRow) throw new Error('Grupo sem titular.');

  const { dateISO, yyyymm } = sysDates;
  ...
  // ✅ FORÇA SYS DATE pra TODOS no array (inclusive titular)
  const dependente = grupoRows.map((row) => {
    ...
    return {
      ...
      // ✅ sysdate
      MMYYYY1Pagamento: yyyymm,
      funcionarioCadastro: 72694,
      dataCadastroLoteContrato: dateISO,
    };
  });
  ...
  return {
    ...
    responsavelFinanceiro: {
      ...
      // sysdate e batendo
      dataApresentacao: dateISO,
      dataAssinaturaContrato: dateISO,
...
async function gerarJsonBeneficiarios() {
  ...
  try {
    ...
    // sysdate gerado UMA VEZ por execução (garante tudo batendo)
    const sysDates = getSysDates();
    log(
      `[BENEF_JSON] Datas forçadas sysdate: dataAssinaturaContrato=${sysDates.dateISO} MMYYYY1Pagamento=${sysDates.yyyymm}`
    );
    ...
    for (const row of rows) {
      ...
      if (!grupos.has(titularId)) grupos.set(titularId, []);
